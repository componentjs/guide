...
Actually you don't need an authenticatoin for GitHub, but then you only get 60 API request per hour. With authenticated requests you get 5.000 per hour.
...
- Since we support semantic versioning, dependencies with `*` fetch the latest published tag now. The master branch is only be used if there are no tags available.
- The __require__ implementation has changed. Before 1.0.0 you could do dynamic require: `require(base + 'someFile')`. This is not possible anymore, because component need to parse all `require()` calls at build time to resolve the paths. Only a string identifier is allowed in the require path.
- The canonical require path has changed. For test environment aliases were registered in 0.19. The pathes to require for the test environment in 1.0.0 are still working, but if you used a non non main component locals, you need the canonical path now. If you have main component like [this](https://github.com/componentjs/boilerplate-express/blob/master/component.json) then the canonical path is `./client/boot`. Non [main files](https://github.com/componentjs/spec/blob/master/component.json/specifications.md#main) of a local need to required with the file name __and__ the file extension, for instance: `./client/boot/helper.js`.
