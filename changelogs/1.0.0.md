...
- Since Component supports semantic versioning, dependencies with `*` fetch the latest published tag now. The master branch is only be used if there are no tags available.
- The __require__ implementation has changed. Before 1.0.0 you could do dynamic require: `require(base + 'someFile')`. This is not possible anymore, because component need to parse all `require()` calls at build time and resolve the paths. Only a string identifier is allowed in the require path.
